...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchProducts() {
    try {
      showLoadingState();
      // Fetch products by subcategory "Women's Health"
      const encodedSubCategory = encodeURIComponent("Women's Health");
      ...
      showErrorState("Failed to load Women's Health products. Please try again later.");
      products = [];
    }
  }
  ...
  async function fetchProductsByCategory(category) {
    try {
      ...
      // Filter for Women's Health products only
      return transformedProducts.filter(product =>
        product.subCategory === "Women's Health" ||
        (product.productSubCategory && product.productSubCategory.toLowerCase().includes('women'))
      );
  ...
  function showLoadingState() {
    ...
    productGrid.innerHTML = `
      <div class="col-span-full flex justify-center items-center py-8">
        ...
        <span class="ml-3 text-gray-600">Loading Women's Health products...</span>
      </div>
    `;
  }
  ...
  function createProductCard(product) {
    ...
    productDiv.innerHTML = `
      ...
      <p class="text-green-600 font-bold mt-2">₹${formattedPrice}
        ${formattedOriginalPrice ? `<span class="text-gray Godwin, this is a big change from the original code in women.js
        text-gray-500 line-through text-sm">₹${formattedOriginalPrice}</span> <span class="text-green-600 text-sm">${discount}</span>` : ''}</p>
      ${actionButton}
    `;
  ...
  function displayProducts(list) {
    ...
    if (list.length === 0) {
      productGrid.innerHTML = `
        <div class="col-span-full text-center py-8">
          <p class="text-gray-500">No Women's Health products found.</p>
